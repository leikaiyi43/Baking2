...
- (UIButton *)createAndConfigureNewButton:(NSString *)buttonTitle withIndex:(NSInteger) index
{
    ...
    if (_layoutStyle == SShorizontalLayout) {
        ...
        while (x <= self.frame.size.width * 2) {
            ...
            for (NSString *buttonTitle in _rollingScrollViewButtonTitles) {
                ...
                UIButton *button = [self createAndConfigureNewButton:buttonTitle withIndex:[_rollingScrollViewButtonTitles indexOfObject:buttonTitle]];
...
- (void)createButtonArrayWithButtonNormalImageNames:(NSArray *) normalImageNames andCenterButtonImageNames:(NSArray *) centerImageNames {
    
    [self createButtonArrayWithButtonTitles:normalImageNames andLayoutStyle:SShorizontalLayout];
    for (int i = 0; i < _rollingScrollViewButtons.count; i++) {
        NSInteger size = [normalImageNames count];
        UIButton *button = [_rollingScrollViewButtons objectAtIndex:i];
        NSLog(@"%@", [normalImageNames objectAtIndex:(i % size)]);
        [button setBackgroundImage:[UIImage imageNamed:[normalImageNames objectAtIndex:(i % size)]] forState:UIControlStateNormal];
        [button setBackgroundImage:[UIImage imageNamed:[centerImageNames objectAtIndex:(i % size)]] forState:UIControlStateSelected];
    }
}
